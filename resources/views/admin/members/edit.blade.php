                ...
                <div class="row form-group">
                    ...
                    <div class="col-md-3">
                        ...
                        <input class="form-control {{ $errors->has('national') ? 'is-invalid' : '' }}" type="text"
                            name="national" id="national" value="{{ old('national', $member->national) }}" {{ config('domains')[config('app.url')]['national_id'] == true ? 'required' : '' }} oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" {{ config('domains')[config('app.url')]['national_id'] == true ? 'min="10" max="10" required' :''}} {{ !is_null($member->parent_phone) ? 'disabled' : '' }}>
